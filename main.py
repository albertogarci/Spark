...
def main_pt():
    ...
    dataset_train, _ = build_imagenet('pt', args.data_path, "custom", args.input_size, eval_crop_pct=None, rrc=args.rrc)
...
def pre_train_one_ep(ep, args, itrt_train, iters_train, model: DistributedDataParallel, optimizer):
    ...
    if not os.path.exists('out/'):
        os.makedirs('out/')
    if not os.path.exists('out/predictions'):
        os.makedirs('out/predictions')

    img.save('out/predictions/img_' + str(ep) + "_" +  str(0) + '.png')
    # Log images to wandb for this epoch
    wandb.log({"predictions_" + str(0): [wandb.Image(img, caption="predictions")]}, step=ep)
